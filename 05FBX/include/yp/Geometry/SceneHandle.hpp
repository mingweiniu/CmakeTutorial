...
namespace yp {
	class SceneHandle {
	public:
		SceneHandle(FbxManager* Manager);
		~SceneHandle();
		FbxScene* getScene();
	...
	SceneHandle::SceneHandle(FbxManager* Manager) :Manager(Manager)
	{
	...
	inline FbxScene * SceneHandle::getScene()
	{
		return Scene;
	}


	void SceneHandle::addNode(FbxNode* Node)
	{
		Root->AddChild(Node);
	}


	bool SceneHandle::Save(std::string PATH)
	{
